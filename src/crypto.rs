...
impl TokenDetails {
    pub fn generate(
        ...
        let token = encode(
            ...
            &claims,
            &EncodingKey::from_rsa_pem(&priv_key).map_err(DynoErr::jwt_error)?,
        )
    ...
    pub fn verify(token: impl AsRef<str>, public_key: impl AsRef<[u8]>) -> DynoResult<Self> {
        use base64::{engine::general_purpose::STANDARD, Engine};
        use jsonwebtoken::{decode, Algorithm, DecodingKey, Validation};
        let pub_key = Engine::decode(&STANDARD, public_key).map_err(DynoErr::any_error)?;
        let token_data = decode::<crate::TokenClaims>(
            token.as_ref(),
            &DecodingKey::from_rsa_pem(&pub_key).map_err(DynoErr::jwt_error)?,
...
#[inline]
pub fn hash_password<Pswd: AsRef<[u8]>>(pswd: Pswd) -> DynoResult<String> {
    let salt = argon2::password_hash::SaltString::generate(&mut rand_core::OsRng);
    argon2::PasswordHasher::hash_password(&argon2::Argon2::default(), pswd.as_ref(), &salt)
        .map(|x| x.to_string())
...
#[cfg(feature = "password_hashing")]
#[inline]
pub fn verify_hash_password<Lhs: AsRef<str>, Rhs: AsRef<[u8]>>(
    pswd_lhs: Lhs,
    pswd_rhs: Rhs,
) -> bool {
...
#[inline]
pub fn checksum_from_bytes(data: impl AsRef<[u8]> + sha256::Sha256Digest) -> String {
    sha256::digest(data)
}

#[cfg(feature = "checksum")]
#[inline]
pub fn compare_checksums(lhs: impl AsRef<[u8]>, rhs: impl AsRef<[u8]>) -> bool {
...
fn test_jwt() {
    ...
    let token_details = TokenDetails::generate(session, 20, PRIVATE_KEY_TESTS);
    assert!(
    ...
    let verify_token = TokenDetails::verify(token_details.token.unwrap(), PUBLIC_KEY_TESTS);
    assert!(
...
fn test_password_hashing() {
    ...
    let hashed = hash_password(pswd);
    assert!(
        matches!(hashed, Ok(_)),
        "the hashing function should not be error"
    );
    assert!(
        verify_hash_password(hashed.unwrap(), pswd),
